...
Storage: Azure SQL (prod); Local dev: Azure SQL in container via Aspire (event store + read projections)
...
\n\n## Observability & Performance Details\n- Aspire orchestrates client/server with OpenTelemetry; Application Insights exporter configured for traces, metrics, logs.\n- Metrics: CreateRide latency (server), client roundtrip; error rate for validation failures.\n- Logs: Structured validation error entries with field and reason.\n- Traces: W3C context propagation across client and API; attributes include userId, rideId, durationMinutes.\n- Projection Lag Target: ≤ 5s from event to read model availability.